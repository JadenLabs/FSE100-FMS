...
class EndPage extends Page {
  ...
  show() {
    ...
    fill(0);
    let oldStars = this.displayStars || 0;
    this.displayStars = Math.min(5, Math.floor(this.displayScore / 500));
    if (this.displayStars > oldStars) {
      shine.play(0, 1, 0.8, 0, 1);
    }
    text(
    ...
    displayStars({
      total: 5,
      remaining: this.displayStars,
      base_x: canvas.m.x - 100,
      base_y: canvas.m.y - 67 + 44,
    }); // 67
